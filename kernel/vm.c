...
void freewalk(pagetable_t pagetable, int ignore_mapping);
...
// newsz, which need not be page aligned. The same physical
// memory will map to kpagetable. Returns new size or 0 on error.
uint64
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, pagetable_t kpagetable)
{
  ...
  for(a = oldsz; a < newsz; a += PGSIZE){
    mem = kalloc();
    if(mem == 0){
      uvmdealloc(pagetable, a, oldsz, kpagetable);
      return 0;
    }
    memset(mem, 0, PGSIZE);
...
// process size. The same pages will be unmapped from kpagetable.
...
void
freewalk(pagetable_t pagetable, int ignore_mapping)
{
  ...
  for(int i = 0; i < 512; i++){
    ...
    if((pte & PTE_V) && (pte & (PTE_R|PTE_W|PTE_X)) == 0){
      ...
      freewalk((pagetable_t)child, ignore_mapping);
      pagetable[i] = 0;
    } else if((pte & PTE_V) && !ignore_mapping){
      panic("freewalk: leaf");
    }
  }
...
int
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz, pagetable_t newkpagetable)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    if(mappages(newkpagetable, i, PGSIZE, (uint64)mem, flags & (~PTE_U)) != 0){
      goto err;
    }
  }
  return 0;
